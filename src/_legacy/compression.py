...
_TOOL_COMPRESSION_STRATEGIES = {
    ...
    "web_search": {
        "preserve_patterns": ["success", "error", "title", "url", "snippet"],
        "truncate_fields": ["content", "body", "text"],
        "max_data_items": 3,
    },
    "web_crawl": {
        "preserve_patterns": ["success", "error", "url"],
        "truncate_fields": ["content"],
        "max_data_items": 0,
    },
}

_PRIORITY_ORDER = ["error", "success", "exit_code", "results_count", "count", "title", "url", "stdout", "stderr", "data", "content"]
...
class ToolOutputCompressor:
    ...
    def compress(self, tool_name: str, output: str) -> str:
        ...
        strategy = _TOOL_COMPRESSION_STRATEGIES.get(tool_name)
        if strategy:
            compressed = self._apply_strategy(output, strategy)
        else:
            compressed = self._generic_compress(output)

        if len(compressed) > self.config.max_chars_per_output:
            compressed = self._enforce_budget(compressed)

        return compressed

    def _apply_strategy(self, output: str, strategy: dict) -> str:
        try:
        ...
        max_items = strategy.get("max_data_items", 5)
    ...
    def _generic_compress(self, output: str) -> str:
        ...
        selected = self.select_lines(lines, self.config.max_lines_per_output)
        header = f"[Output compressed: {len(lines)} lines -> {len(selected)} lines]"
        return header + "\n" + "\n".join(selected)

    def select_lines(self, lines: list[str], budget: int) -> list[str]:
        if len(lines) <= budget:
            return lines
    ...
    def _enforce_budget(self, text: str) -> str:
        if len(text) <= self.config.max_chars_per_output:
            return text

        truncated = text[:self.config.max_chars_per_output - 50]
